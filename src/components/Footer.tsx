    ...
    <footer className="bg-gradient-dark text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          <div className="space-y-6">
            <h4 className="font-poppins font-semibold text-white">Certifications</h4>
            <div className="grid grid-cols-2 gap-4">
              {certifications.map((cert, index) => (
                <div key={index} className="flex flex-col items-center text-center p-3 bg-white/5 rounded-lg hover:bg-white/10 transition-colors">
                  <div className="text-magenta-400 mb-2">
                    {cert.icon}
                  </div>
                  <span className="text-xs text-gray-300 font-inter font-medium">
                    {cert.text}
                  </span>
                </div>
              ))}
            ...
            {/* Legal Links */}
            <div className="mt-8">
              <h4 className="font-poppins font-semibold text-white mb-3">Legal</h4>
              <div className="space-y-2">
                {legal.map((item) => (
                  <Link
                    key={item.name}
                    to={item.href}
                    className="block text-sm text-gray-300 hover:text-magenta-300 transition-colors font-inter"
                  >
                    {item.name}
                  </Link>
                ))}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="md:flex md:items-center md:justify-between">
            <div className="text-sm text-gray-400 font-inter">
              © {currentYear} HealthSpan360. All rights reserved.
            </div>
            <div className="mt-4 md:mt-0">
              <p className="text-xs text-gray-500 font-inter">
                Licensed medical professionals only. All testing requires provider authorization.
              </p>
            </div>
          </div>
        </div>
